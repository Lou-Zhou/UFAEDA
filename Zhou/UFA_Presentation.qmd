...
drawEndLocs <- function(successful = "Both", year = "Keep All", distanceBounds = c(0, 100), showContour = TRUE, showPoints = FALSE){
...
if(player != "Keep All"){
  title = paste(title, "| Year", year)
}
plot_ufa_throws <- ufa_throws |> 
  dplyr::mutate(
    year = as.numeric(substr(gameID, start = 1, stop = 4))
  ) |>
  dplyr::filter(
    (successful != "True" | turnover == 0),
    (successful != "False" | turnover == 1),
    (year == "Keep All" | year == year),
...
-   Throw Data originating from the 2021-2024 seasons of the Ultimate Frisbee Association(UFA)
    -   Physical Throw Data
...
## Throw Distance Analysis {.smaller}
...
## Developing Player Archetypes Using K-Means Clustering {.smaller}
